# B站动态监控系统 (BTCE 4.8) 项目说明

## 项目简介
BTCE 4.8 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令实时更换置顶动态ID + 远程更新B站Cookie（邮箱二维码→扫码→自动保存）。v4.8 新增页面复用性能优化、滚动优化、检查间隔调优，8000轮从19.4h降至13.3h。监控目标通过 `dynamic.py` 配置。
...
| 文件 | 作用 |
|------|------|
| `monitor.py` | Monitor 类：Playwright 浏览器驱动，v4.8 页面复用（每轮不再 new_page/close），主循环 ①置顶评论变更检测 ②API 动态列表发现新动态 ③记录 API 健康统计 ④按 QQ_MODE/EMAIL_MODE/BILI_MODE 分流推送 ⑤B站自动发布 |
...
main.py 启动
  ...
  │   ├─ 动态监控 (Monitor.run)
  │   │   └─ 每6秒循环:
  │   │       ├─ Playwright 浏览器（v4.8页面复用）打开置顶动态页 → 抓取置顶评论 → 变更检测
...
CHECK_INTERVAL = 6                                # 动态检查间隔（秒，v4.8页面复用后缩短）
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.8
- **更新内容**：页面复用（~3s/轮）+ 滚动优化（5→3次）+ CHECK_INTERVAL 8→6s + BROWSER_RESTART 10→100轮 + B站推送评论直达链接（rpid穿透Shadow DOM），8000轮 19.4h→13.3h
- **Python 要求**：3.8+
- **最后更新**：2026年7月13日
