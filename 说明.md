# B站动态监控系统 (BTCE 4.14) 项目说明

## 项目简介
BTCE 4.14 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 + 置顶自动识别 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令（更换置顶、切换手动/自动模式、更新Cookie）。v4.14 新增置顶动态自动发现（API module_tag识别）+ 手动/自动双模式 + 更换截图邮件通知。
...
├── bili_api.py              # B站新版 polymer API 客户端（v4.14: +_fetch_raw()原始数据+module_tag）
├── pinned_dynamic_monitor.py # 置顶动态自动发现+手动/自动双模式（v4.14）
...
│
├── config_custom.example.py # 个性化文案配置示例（v4.14: config_custom模式）
...
| 文件 | 作用 |
|------|------|
| `monitor.py` | Monitor 类：Playwright 浏览器驱动，主循环 ①置顶评论变更检测 ②API 动态列表发现新动态 ③定期检测置顶是否更换(截图+邮件通知) ④按 QQ_MODE/EMAIL_MODE/BILI_MODE 分流推送 ⑤B站自动发布。v4.14 新增 _take_dynamic_screenshot() 公用截图、置顶更换gradient卡片邮件 |
...
| `qq_callback_server.py` | NapCat HTTP 事件回调服务器（@机器人 更换置顶/切换手动/切换自动/测试/更新凭证）v4.14 新增模式切换+状态显示B站置顶vs监测动态 |
...
main.py 启动
  ├─ Application.run()
  │   ├─ 动态监控 (Monitor.run)
  │   │   └─ 每N秒循环 (config_custom可配置):
  │   │       ├─ ① Playwright 浏览器 打开置顶动态页 → 抓取置顶评论 → 变更检测
  ...
  │   │       ├─ ② BiliAPI 获取动态列表 → 对比 seen_dynamics → 类型过滤 → 新动态截图 → 邮件+QQ
  │   │       │   └─ → health_checker.increment_api_success/failure (API独立统计)
  │   │       ├─ ③ 定期检测置顶是否更换 (PINNED_CHECK_INTERVAL)
  │   │       │   ├─ API module_tag 识别B站置顶
  │   │       │   ├─ 自动模式: 更换→更新目标→截图→gradient邮件通知
  │   │       │   └─ 手动模式: API照查但不自动切换(QQ回调可切)
  │   │       └─ 性能记录 (performance_monitor.record_cycle → P1/P2 + 日报双通道)
  │   │
...
### 4. 配置通知方式
- 复制 `config_custom.example.py` → `config_custom.py`，填写个性化文案（UP主昵称、QQ消息文案、邮件标题等），不填则用通用默认值
- 复制 `config_email.example.py` → `config_email.py`，填写SMTP信息
- 复制 `config_qq.example.py` → `config_qq.py`，填写QQ群ID

### 5. 添加监控目标
编辑 `config_custom.py`（不存在则复制 `config_custom.example.py`）：
```python
UP_UID = "目标UID"
UP_NAME = "目标名称"
```
...
# 监控目标
UP_UID = ""                                     # B站UID
PINNED_DYNAMIC_ID = ""                             # 置顶动态ID（v4.14起自动发现，可手动覆盖）
CHECK_INTERVAL = 6                                # 动态检查间隔（秒）
PINNED_CHECK_INTERVAL = 3600                      # 置顶更换检测间隔（秒，默认1h）

# 个性化配置（config_custom.py）
# 任何变量在 config_custom.py 中定义后自动覆盖 config.py 默认值
# 包括: UP_NAME, CHECK_INTERVAL, QQ消息文案, 邮件标题等
...
BROWSER_RESTART_INTERVAL = 100                    # v4.8页面复用后减少重启频率
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.14
- **更新内容**：置顶动态自动发现(API module_tag) + 手动/自动双模式(QQ回调切换) + 更换截图邮件通知 + config_custom配置分离
- **Python 要求**：3.9+（服务器3.9，本地3.13，代码须兼容3.9）
- **最后更新**：2026年7月27日
