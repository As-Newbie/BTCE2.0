# B站动态监控系统 (BTCE 4.9) 项目说明

## 项目简介
BTCE 4.9 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令实时更换置顶动态ID + 远程更新B站Cookie（邮箱二维码→扫码→自动保存）。v4.8 页面复用性能优化（8000轮 19.4h→13.3h），v4.9 B站推送评论直达链接（rpid穿透Shadow DOM）。监控目标通过 `dynamic.py` 配置。
...
- **项目名称**：B站动态监控系统 (BTCE)
- **当前版本**：v4.9
- **更新内容**：rpid穿透三层Shadow DOM + B站推送`#reply{rpid}`双链接 + dry-run调试模式 + `@机器人 测试`显示当前置顶
- **Python 要求**：3.8+
- **最后更新**：2026年7月13日
