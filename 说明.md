# B站动态监控系统 (BTCE 4.13) 项目说明

## 项目简介
BTCE 4.13 是基于 Python + Playwright 的 B站动态/直播监控系统。核心流程：API 动态列表发现 → Playwright 浏览器截图 → 邮件/QQ/B站三通道推送（模式可配）。支持 QQ 群 @机器人 指令实时更换置顶动态ID + 远程更新B站Cookie（邮箱二维码→扫码→自动保存）。v4.13 新增动态类型过滤（排除B站自动生成的直播推荐动态）。
